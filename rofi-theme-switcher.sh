...
apply_theme() {
	...
	# Crop a portrait slice from the center of the wallpaper for rofi sidebar
	convert "$wallpaper" -gravity Center -crop 600x1080+0+200 +rep3age ~/.cache/wal/rofi-preview.jpg
	...
	# Reload i3/sway (if using pywal colors in config)
...
show_menu() {
	...
	# Create desktop entry style format for rofi with icons
	>"$preview_file"
	...
	while IFS= read -r wallpaper; do
		...
		# Verify thumbnail exists before adding to list
		if [ -f "$thumbnail" ]; then
			wallpapers+=("$name")
			wallpaper_paths+=("$wallpaper")

			# Create entry with thumbnail path as icon
